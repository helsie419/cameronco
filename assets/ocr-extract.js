...
/* Insurer portal screenshots are usually a browser window captured at native
   screen resolution — the *overall* image can be large while individual
   field text is still only 12-14px tall, well under the ~30px glyph height
   Tesseract's LSTM engine wants for best accuracy. Upscaling only when the
   whole image is small (the old check) misses that case entirely, so this
   always scales up by a healthy factor, capped to keep processing time
   reasonable on already-large screenshots. Contrast is boosted only mildly —
   aggressive contrast clips the anti-aliased edges of small text, which the
   LSTM engine actually reads better than a harshly thresholded image. Falls
   back to the original image if canvas processing isn't available. */
async function ocrExtractPreprocessImage(image) {
  const bitmap = await createImageBitmap(image);
  try {
    const scale = Math.min(2.5, 6000 / bitmap.width);
