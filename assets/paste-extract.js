...
const PASTE_EXTRACT_LABELS = {
  name: ['name', 'claimant', 'claimant name', 'insured', 'insured name', 'customer name', 'policy holder', 'policyholder'],
  email: ['email', 'email address', 'e mail'],
  phone: ['phone', 'mobile', 'mobile number', 'phone number', 'contact number', 'contact phone', 'telephone'],
  address: ['address', 'risk address', 'property address', 'postal address', 'residential address', 'site address'],
  suburb: ['suburb', 'city', 'town'],
  state: ['state'],
  postcode: ['postcode', 'post code', 'zip'],
  claim_number: ['claim number', 'claim no', 'claim ref', 'claim reference', 'claim id'],
  policy_ref: ['policy number', 'policy no', 'policy ref', 'policy reference'],
...
function parsePastedInsurerText(text, insurerOptions) {
  const map = pasteExtractParseLabeledLines(text);
  const fields = {};
  const matched = [];
