...
```
db/schema.sql                  Postgres schema (portable — 13+, no Neon-specific features)
db/seed.sql                    Staff, insurers, lookups, rate cards & watch brand rates
...
Running on **Netlify's Postgres (Neon)** now, moving to a self-hosted
Postgres server later. The schema is deliberately plain SQL —
`BIGINT GENERATED ALWAYS AS IDENTITY`, standard indexes, standard triggers —
nothing Neon-specific, so the move later is just a `DATABASE_URL` change and a
`pg_dump` / `pg_restore`.

```bash
netlify db init
npm run db:setup     # applies db/schema.sql then db/seed.sql via DATABASE_URL
```

(`db/setup.mjs` does the same thing as `psql "$DATABASE_URL" -f db/schema.sql`
followed by `seed.sql`, without needing `psql` installed.) `seed.sql` is
idempotent — safe to re-run, including the trickier same-day-versioning edge
cases in the rate cards (see §2). `schema.sql` is meant to run once against a
fresh database.

When you move to the client's own server: point `DATABASE_URL` at it, run the
same command, `pg_dump --data-only` from Neon and load in. No code changes.
...
| Variable | Purpose |
|---|---|
| `DATABASE_URL` *(or `NETLIFY_DATABASE_URL`)* | Postgres connection string |
...
- Local database-backed testing, without deploying:
  ...
  Netlify Function files, so it reads the configured Netlify Postgres database
  without pushing changes to Netlify. Do not open the HTML files directly when
  you want database-backed testing — direct file preview cannot call `/api/*`.
...
Ran against a real local Postgres 16 instance throughout (not mocked):
